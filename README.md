...
> In this template, we use `@omnifex` as a placeholder for package names. As a user, you might want to replace it with your own organization or project name. You can use find-and-replace to change all the instances of `@omnifex` to something like `@my-company` or `@project-name`.
...
pnpm --filter @omnifex/auth generate
```
...
3. Outputs a Drizzle-compatible schema file to the `@omnifex/db` package
