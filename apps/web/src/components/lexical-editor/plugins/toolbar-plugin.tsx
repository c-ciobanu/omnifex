...
export function ToolbarPlugin({ documentId, saveDisabled }: Props) {
  ...
  const updateDocumentMutation = useMutation(
    orpc.documents.update.mutationOptions({
      onSuccess: async () => {
        await queryClient.invalidateQueries(orpc.documents.get.queryOptions({ input: { id: documentId } }));
  ...
  function onSaveClick() {
    ...
    updateDocumentMutation.mutate({ id: documentId, body: jsonString });
  }
    ...
    <>
      <div className="sticky top-0 z-10 mb-px flex space-x-1 overflow-auto rounded-t-md bg-white p-1">
        <Toggle
          ...
          onClick={onSaveClick}
          pressed={false}
          disabled={saveDisabled || updateDocumentMutation.isPending}
        >
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Toggle
              ...
              size="sm"
              pressed={false}
              className="shrink-0 gap-2 font-normal whitespace-nowrap"
            >
        ...
        <DialogContent>
          ...
          >
            <FieldGroup>
              <form.AppField
                name="url"
                children={(field) => <field.InputField label="Link" inputProps={{ type: "url" }} />}
              />
            </FieldGroup>
          </form>
          ...
          <DialogFooter>
            ...
            <form.AppForm>
              <form.SubmitButton form="link-form">Save</form.SubmitButton>
            </form.AppForm>
          </DialogFooter>
        </DialogContent>
      </Dialog>
